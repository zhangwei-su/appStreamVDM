...
import static com.google.location.nearby.apps.walkietalkie.VDMController.STREAMING_APP_DPI_SCALING;
...
public class MainActivity extends ConnectionsActivity {
  ...
  private final GestureDetector mGestureDetector =
      new GestureDetector(KeyEvent.KEYCODE_VOLUME_DOWN, KeyEvent.KEYCODE_VOLUME_UP) {
        ...
        protected void onHold() {
          logV("GestureDetector onHold");
          //startRecording();
          startAppStreaming(new ComponentName("com.google.android.calculator", "com.android.calculator2.Calculator"));
        }

        @Override
        protected void onRelease() {
          logV("GestureDetector onRelease");
          //stopRecording();
          //stopAppStreaming();
        }
      };
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    mHandler.post(() -> initialCDM());
    createReceiverView();
  }
  private void initialCDM() {
    mCDM = (CompanionDeviceManager) getSystemService(Context.COMPANION_DEVICE_SERVICE);
    mCDM.associate(
            new AssociationRequest.Builder()
                    //.setDeviceProfile(DEVICE_PROFILE_NEARBY_DEVICE_STREAMING)
                    .setDeviceProfile(DEVICE_PROFILE_APP_STREAMING) //can't go though CompanionDeviceActivity.java
                    ...
                    .build(),
            getMainExecutor(), mCDM_Callback);
  }

  private static final int REQUEST_CODE_ASSOCIATE = 101;
  private CompanionDeviceManager.Callback mCDM_Callback = new CompanionDeviceManager.Callback() {
    @Override
    public void onAssociationPending(@NonNull IntentSender intentSender) {
      logV("onAssociationPending " + intentSender);
    ...
    public void onAssociationCreated(@NonNull AssociationInfo associationInfo) {
      logV("onAssociationCreated " + associationInfo);

      mAssociationId = associationInfo.getId();

      mVDMController = new VDMController(MainActivity.this, mAssociationId);
    }
  ...
  private void createReceiverView() {

    FrameLayout container = (FrameLayout) findViewById(R.id.debug_log).getParent();
    TextureView tView = new TextureView(this);//findViewById(R.id.CastView);
    tView.setPivotX(0);
    tView.setPivotY(0);
    FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(STREAMING_APP_WIDTH, STREAMING_APP_HEIGTH);
    tView.setLayoutParams(layoutParams);
    tView.setOpaque(true);
    tView.setSurfaceTextureListener(mSurfaceTextureListener);
    Matrix matrix = new Matrix();
    matrix.setScale(STREAMING_APP_DPI_SCALING, STREAMING_APP_DPI_SCALING);
  ...
  private final TextureView.SurfaceTextureListener mSurfaceTextureListener =
          new TextureView.SurfaceTextureListener() {
            ...
            public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture,
              ...
              mSSRender = new StreamSurfaceRender(new Surface(surfaceTexture));
            }
            ...
            public void onSurfaceTextureSizeChanged(SurfaceTexture surfaceTexture,
                                                    int width, int height) {
              logD("onSurfaceTextureSizeChanged");
            }
  ...
  protected void onReceive(Endpoint endpoint, Payload payload) {
    if (payload.getType() == Payload.Type.STREAM) {
      mSSRender.startDecoder(payload.asStream().asInputStream());
      /*
  ...
  private VDMController mVDMController;
  private CompanionDeviceManager mCDM;
  private void startAppStreaming(ComponentName appName) {
    logV("startAppStreaming()");
    PipedOutputStream pipedOutputStream = new PipedOutputStream();

    mVDMController.prepareDisplay();
    int displayId = mVDMController.getVirtualDisplay().getDisplay().getDisplayId();
    ...
    logD("starting test app, displayId:" + displayId);
    startActivity(intent, options.toBundle());
    //catching from VD, send to peer

    try {
      PipedInputStream pipedInputStream = new PipedInputStream();
      pipedInputStream.connect(pipedOutputStream);
      send(Payload.fromStream(pipedInputStream));
      mVDMController.start(pipedOutputStream);
    } catch (IOException e) {
      logE("startAppStreaming() failed", e);
    }
  }
  private void stopAppStreaming() {
    mVDMController.stop();
  }
